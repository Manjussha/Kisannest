...
import { LoadingController, NavController, Platform, ToastController,AlertController } from '@ionic/angular';
...
export class CheckoutPage implements OnInit {
    ...
    minDate: string = new Date(new Date().setDate(new Date().getDate() + 0)).toISOString();
    maxDate: string=new Date(new Date().getTime()+(2*24*60*60*1000)).toISOString();
        constructor(private oneSignal: OneSignal, public toastController: ToastController, 
        public platform: Platform, public api: ApiService, public checkoutData: CheckoutData, 
        public settings: Settings, public router: Router, public iab: InAppBrowser, 
...
const stringified = JSON.stringify(currentdate);
this.current_date = stringified.substring(9, 11);

        
        //var maxDate 1= new Date();
        //var  maxDate2 = maxDate.setDate(maxDate.getDate() + 1);
        //this.maxDate = maxDate2.toISOString();
    ...
    async placeOrder() {
        ...
        var parseDate = this.parseISOString(this.checkoutData.form.delivery_date)
        var date  =  this.isoFormatDMY(parseDate)
        console.log(date);
        this.checkoutData.form.coderockz_woo_delivery_date_field = date;
    ...
    const date = stringified.substring(9, 11);
    console.log(date);
    var d = new Date();
let hours=d.getHours();
  hours = hours; // the hour '0' should be '12'
  console.log(hours);
...
if(this.current_date == date){
   if(hours<7){
    ...
    },
    {
        ...
        "stime":"17",
        "value":"1020,1140",
        "label":"5:00 PM to 7:00 PM"
    },
    {
        "id":"7",
        "stime":"19",
        "value":"1140,1260",
        "label":"7:00 PM to 9:00 PM"
    }];
   }
    ...
    const alert = await this.alertController.create({
        ...
        header: 'kisannest',
        message: 'No delivery slot available for today',
        buttons: ['OK']
      });
    
      await alert.present();
this.timeRangeData.length=0;
